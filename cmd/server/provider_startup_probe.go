...
func applyStartupProviderProbeSuggestions(ctx context.Context, cfg *config.Config, client *http.Client) error {
	...
	if len(cfg.Upstreams) == 0 {
		return applyStartupProviderProbeToTarget(ctx, &config.UpstreamTargetConfig{
			ID:       "default",
	...
	for i := range cfg.Upstreams {
		...
		if err := applyStartupProviderProbeToTarget(ctx, target, &target.Upstream, client); err != nil {
			return err
		}
	}
	return nil
...
func applyStartupProviderProbeToTarget(ctx context.Context, target *config.UpstreamTargetConfig, upstreamCfg *config.UpstreamConfig, client *http.Client) error {
