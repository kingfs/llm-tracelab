...
func buildDoctorResult(opts doctorOptions) doctorResult {
	...
	result.Checks = append(result.Checks, checkDoctorResponsesServer(cfg))
...
func checkDoctorResponsesServer(cfg *appconfig.Config) doctorCheck {
...
func checkDoctorProviderProbe(probeProviders bool) doctorCheck {
	...
	return doctorCheck{
		...
		Status:  doctorStatusWarn,
		Message: "provider probe is requested but doctor first cut keeps network probing out of startup diagnostics",
