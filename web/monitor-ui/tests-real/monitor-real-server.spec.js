...
test("real monitor server supports local secret key operations", async ({ request }) => {
  const before = await request.get("/api/secrets/local-key");
  expect(before.ok()).toBeTruthy();
  const rotation = await request.post("/api/secrets/local-key?rotate=1");
  expect(rotation.ok()).toBeTruthy();
  const result = await rotation.json();
  expect(result.new_fingerprint).toBeTruthy();
  expect(result.new_fingerprint).not.toBe(result.old_fingerprint);
});
...
test("real monitor server renders routing decision records", async ({ page }) => {
  ...
  const exchanges = await page.request.get("/api/routing/exchanges").then((response) => response.json());
  expect(exchanges.items).toHaveLength(2);
  expect(exchanges.items.every((item) => item.selected_upstream_id === "openai-primary")).toBeTruthy();
