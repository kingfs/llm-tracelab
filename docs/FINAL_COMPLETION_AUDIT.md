...
| Local server-mode does not pass `/v1/responses` through when enabled | Smoke showed local Responses JSON/SSE produced while upstream only implemented `/v1/chat/completions`; router logs selected Chat Completions target. |
...
| Unsupported MCP/file/code/computer-use tools do not fake execution | Runtime rejects unsupported hosted tools with `unsupported_tool`; local smoke verified forced MCP returns HTTP 400 and writes a `tool_call_audits` row with `status=rejected`. |
...
- `rtk docker compose --profile search config`
...
- Fresh Postgres local smoke: Responses compact, unsupported MCP rejected audit, non-Responses Chat Completions proxy recording, and `pkg/replay` offline replay.
...
- Real MCP/file/code/computer-use hosted tool execution lifecycle.
