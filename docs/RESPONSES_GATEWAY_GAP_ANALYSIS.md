...
- 剩余缺口：尚未从数据库 catalog/channel 合并 profile 参数；provider/channel profile 成为 runtime budget/profile 事实源前，还需要实现 schema migration、rollback plan 和 DSN-gated tests。当前 report 已把这些列为 blocking gates，但仍只是 observe-only，不代表可以采纳。
