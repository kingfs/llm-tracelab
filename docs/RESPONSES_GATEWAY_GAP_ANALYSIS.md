...
- 缺口：auto compact 后 hosted/未知工具、非平凡 `tool_choice` 和复杂混合工具 lifecycle 仍 fallback；compact provenance 当前仍以内嵌 response metadata 和 execution events 派生 read model 为主，尚无独立 schema、HTTP/CLI 查询 surface；artifact-bearing item 策略和完整 context optimization 未接入。
...
- 已吸收首切：`llm-tracelab models codex-config <model>` 离线读取 `responses_server.model_profiles`，输出 `models.codex_config` JSON envelope、Codex TOML 建议、provider `base_url` 推导、`wire_api=responses`、profile match/compact threshold diagnostics 和 warnings；diagnostics 默认输出 `runtime_profile_source=responses_server.model_profiles`、`profile_precedence=[responses_server.model_profiles, zero_limits_when_unmatched]`、`catalog_profile_role=available_for_runtime_opt_in`、`capability_source=provider_upstream_capabilities`、`provider_channel_profile_adoption=report_only`、`profile_adoption_report`、`profile_conflict_strategy=responses_server.model_profiles_wins` 和 required gates。本地 SQLite app DB 文件可用时，会以 `AutoMigrate:false` 打开 store 并只读检查 `model_catalog` / `channel_models` 中是否存在请求模型，输出 `catalog_model_present`、`channel_model_present`、`channel_model_count`、source 状态和 drift warnings；`profile_adoption_report` 是机器可读 dry-run/conflict/gate report，展示 channel profile candidate、`context_window_tokens`、`max_output_tokens`、`compact_history_item_threshold`、`upstream_model`、显式 `responses_server.model_profiles` 保护、`supports_chat_completions=false` 阻断和 `mutates=false`，并输出 `adoption_ready=true`、`blocking_gate_count=0`，其中 `schema_migration` 为 `implemented_runtime_opt_in`，其余 gate 为 `implemented_contract`。显式设置 `responses_server.adopt_channel_model_profiles=true` 后，runtime 与 `models codex-config` 会在无显式 profile、无冲突且 capability 允许时采用 adopted channel model profile，并输出 `runtime_profile_source=channel_models.profile_adoption` 与 `profile_adoption_report.status=adopted_runtime`；显式传入 `--codex-config <path>` 时，还会只读解析本地 Codex TOML，检查 `[profiles.<model>]`、`[model_providers.llm-tracelab]` 和 profile/provider 关键字段 drift。
...
- 剩余缺口：provider/channel profile 已能通过 `responses_server.adopt_channel_model_profiles=true` 显式 opt-in 进入 runtime 和 Codex config 建议，但默认仍不采纳，也不会改写配置或数据库。后续缺口是把 adoption 状态管理、冲突解释、禁用/回滚和 Monitor/CLI 操作面补齐，并扩大 Postgres DSN-gated 覆盖。
