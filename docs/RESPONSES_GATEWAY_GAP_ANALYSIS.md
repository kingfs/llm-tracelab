...
- 已有：Postgres application `db migrate up/status/down --dry-run`、auth migrate、SQLite status 解释、open-vs-migrate 分离；`auth migrate status` / dry-run 已输出 auth required tables、table health、Postgres shared namespace strategy 和 independent auth namespace `not_implemented` 方案边界。
- 缺口：SQLite 仍是 startup schema fallback，不是 versioned migration；auth 仍共享 application schema namespace，独立 auth migration namespace 未完成且当前仅通过 status/dry-run 明确约束。
