...
- 剩余缺口：尚未从数据库 catalog/channel 合并 profile 参数；provider/channel profile 成为 runtime budget/profile 事实源前，还需要实现 schema migration 和 DSN-gated tests。当前 report 已把 rollback plan 固化为 mutation-free contract，但仍只是 observe-only，不代表可以采纳。
