...
- 剩余缺口：尚未从数据库 catalog/channel 合并 profile 参数；provider/channel profile 成为 runtime budget/profile 事实源前，还需要实现 schema migration、dry-run diff、conflict report、rollback plan 和 DSN-gated tests，并把 observe-only 状态推进到可控 adoption。
