...
- 剩余缺口：provider/channel profile 已能通过 `responses_server.adopt_channel_model_profiles=true` 显式 opt-in 进入 runtime，但默认仍不采纳，`models codex-config` 仍只是 observe-only dry-run，不会改写配置或数据库。后续缺口是把 adoption 状态管理、冲突解释、禁用/回滚和 Monitor/CLI 操作面补齐，并扩大 Postgres DSN-gated 覆盖。
