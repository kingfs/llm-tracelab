...
- `provider probe`：手动诊断命令，会对配置中的 upstream endpoint 做保守探测并输出建议的 `api_type`、`protocol_family` 和 capability signals。该结果不会写回配置，也不会在启动时覆盖显式配置。
