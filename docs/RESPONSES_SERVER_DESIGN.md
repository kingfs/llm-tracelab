...
- provider auto-detect；当前已有手动 `provider probe` 诊断建议，provider capability 仍需显式配置或由已有渠道/模型数据表达。
