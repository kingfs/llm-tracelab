...
- `tool_call_audits`：面向 hosted/server-side tool lifecycle 的可索引 read model，字段覆盖 response/request/conversation/call/tool/status/input/output/error/metadata/timestamps。当前 schema、migration、recorder 和 query API 已有；runtime 还未把所有 web_search/executor/unsupported hosted tool lifecycle 双写进该表。
