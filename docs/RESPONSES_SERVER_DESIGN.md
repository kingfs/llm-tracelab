...
- model profile 驱动的 tokenizer/context optimization；当前 HTTP provider `/tokenize` counter 已可通过 `responses_server.model_profiles[].tokenize_counter.enabled=true` 显式接入 proxy/runtime 装配，要求目标声明 `capabilities.tokenize=true`，但尚未自动选择或默认启用。
