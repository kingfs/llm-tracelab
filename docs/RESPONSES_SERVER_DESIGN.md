...
6. 当前 Runtime 输出 OpenAI Responses 兼容 response；`stream:true` 由 HTTP handler 以 deferred SSE envelope 输出。下游请求 stream 时，内部 model call 已请求 OpenAI-compatible Chat Completions SSE、记录原始 SSE cassette，并聚合成完整 response；下游边读边转发仍未完成。
