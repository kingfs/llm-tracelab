...
6. 当前 Runtime 输出 OpenAI Responses 兼容 response；`stream:true` 对简单文本输出会由 HTTP handler 边读取内部 Chat Completions SSE、边输出 Responses `response.output_text.delta`；普通 function tool call 参数分片会输出 `response.function_call_arguments.delta/done`，完成后存储完整 response。已注册 server-side function executor 的 stream 首切会执行工具、先输出 started 态 `response.output_item.added` tool item、成功后输出完成态 `response.output_item.done` tool item 并继续下一轮最终文本 delta，并写带 `stream=true` 的 tool_call lifecycle events；失败时会 best-effort 输出 failed `response.output_item.done`，再由 HTTP 层追加 `response.failed`。provider 就绪的 hosted `web_search` stream 首切也会执行 server-side search、输出 started 态 `response.output_item.added` 和完成/失败态 `response.output_item.done` `web_search_call` item、注入结果并继续最终文本 delta。已写出 SSE 后的 runtime 错误会尽量追加最小 `response.failed` SSE；未写出 SSE 的错误仍走原 HTTP error 或 deferred fallback 行为。auto compact 等复杂路径使用 deferred SSE envelope；更完整的多工具 failed lifecycle events 仍未完成。内部 model call 会记录原始 OpenAI-compatible Chat Completions SSE cassette。
...
当前状态：Stage 18A 已让 `POST /v1/responses` 的 `stream:true` 返回 `text/event-stream`，事件覆盖 `response.created`、`response.in_progress`、message output text delta/done、function call arguments delta/done、output item added/done 和 `response.completed`，并写入 `response.stream` fallback/started/completed execution events。Stage 18B 已在 `internal/responses/chatclient` 增加 OpenAI-compatible Chat Completions SSE 聚合，能把文本 delta、function tool call delta 和 usage trailer 聚合为内部 `ChatCompletionResponse`；Responses server-mode 的 stream 请求也会让内部 Chat Completions model call 请求上游 SSE，并把原始 SSE 记录为 stream cassette。Stage 18C 已把 context cancellation 记录为 `cancelled` request audit 与 request/model_call execution events，并让这些 audit 写入使用不随客户端断开的 context。Stage 20 已新增 `CreateStream` 首切和 Chat SSE callback，简单文本输出路径可边接收上游边输出下游 `response.output_text.delta`，完成后仍存完整 response；普通 function tool call argument 分片也会输出 `response.function_call_arguments.delta/done`。已注册 server-side function executor 的 stream 首切支持执行工具、先输出 started 态 `response.output_item.added` tool item、成功后输出完成态 `response.output_item.done` tool item 并进入下一轮最终文本 delta，失败时输出 failed `response.output_item.done`，且 tool_call lifecycle events 会带 `stream=true`。provider 就绪的 hosted `web_search` stream 首切也支持执行 hosted search、输出 started 态 `response.output_item.added` 和完成/失败态 `response.output_item.done` `web_search_call` item、注入工具结果、继续最终文本 delta，并写带 `stream=true` 的 tool_call events。增量 stream 已写出 SSE 后的 runtime 错误会尽量追加最小 `response.failed` SSE，并继续记录 `response.stream` failed audit；auto compact 等复杂路径仍会 fallback 到 deferred streaming；更完整的多工具 failed lifecycle 仍未完成。
