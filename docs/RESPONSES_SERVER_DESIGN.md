...
- 完整 Postgres migration 生产化。当前已有 checked-in 初始 SQL，Postgres `db migrate up` 会应用版本化 SQL，application store 已拆分 open-vs-migrate，并完成 migrated `logs` 路径的首轮 raw SQL 兼容；剩余缺口是 SQLite 应用迁移仍未版本化、Postgres auth migration 未完成、Postgres migration 尚未覆盖全部 SQLite raw DDL 表、raw SQL 兼容性仍需持续审计。
