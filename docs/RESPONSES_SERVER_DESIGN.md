...
TraceLab 当前的核心价值是把真实 LLM HTTP exchange 录制为可检查、可回放的 `.http` cassette，并用 Postgres-first application DB 建立 Monitor/MCP 所需的生产索引、配置、审计和 Responses semantic state。SQLite 仅作为本地开发、离线测试和既有本地 DB 兼容 fallback。Responses server-mode 在保留 record/replay 能力链的基础上，为 Codex、OpenAI Responses API 客户端和本地 OpenAI-compatible Chat Completions 模型提供 Responses 语义服务端。
...
- SQLite 仍需兼容本地开发、离线测试和既有本地 DB fallback。生产 Monitor/MCP 查询源是 Postgres-backed application DB；旧 replay 或旧 `.http` 文件不能依赖 Postgres。
...
- Provider/channel `model_profiles` 可以通过显式 opt-in 成为 Responses Runtime 构建 context、compact 和上游模型 rewrite 的补充事实源；默认仍以 `responses_server.model_profiles` 为 runtime profile source，匹配失败时按 zero limits 回落。`models codex-config` 会把这一优先级作为 `runtime_profile_source` / `profile_precedence` 输出；默认标记 `catalog_profile_role=available_for_runtime_opt_in` 与 `provider_channel_profile_adoption=report_only`，开启 `responses_server.adopt_channel_model_profiles=true` 且存在无冲突 adopted channel profile 时输出 `runtime_profile_source=channel_models.profile_adoption`、`provider_channel_profile_adoption=runtime_opt_in` 和 `profile_adoption_report.status=adopted_runtime`。`profile_adoption_report` 内 `schema_migration` 为 `implemented_runtime_opt_in`，`dry_run_diff`、`conflict_report`、`rollback_plan` 和 `dsn_gated_tests` 均为非阻塞 implemented contract 状态；CLI 不修改数据库，回滚通过关闭 opt-in 或移除 adopted profile marker 完成。
...
当前状态：ent schema、SQLite raw DDL fallback、`runtime.NewEntStore`、Postgres 打开路径、Postgres 应用库 `db migrate up` versioned SQL 路径、application store open-vs-migrate 分离、application raw DDL 表覆盖补齐、migrated logs/observation/finding/analysis/system-event/eval/experiment/monitor/routing/model/channel 代表路径、request audit 写入、内部 Chat Completions cassette 的 upstream exchange correlation、request/model_call/hosted web_search/tool rejected execution/audit events，以及核心 audit 查询服务/Monitor API/MCP/UI 查询工具已落地。Responses ent store 属于应用库；Postgres auth 表当前由 application migration set 拥有，`auth migrate down` 被阻止，独立 auth migration namespace 另行处理。
