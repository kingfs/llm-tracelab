...
6. 当前 Runtime 输出 OpenAI Responses 兼容 response；`stream:true` 对简单文本输出会由 HTTP handler 边读取内部 Chat Completions SSE、边输出 Responses `response.output_text.delta`；普通 function tool call 参数分片会输出 `response.function_call_arguments.delta/done`，完成后存储完整 response。hosted tools、server-side tool execution、auto compact 等复杂路径仍使用 deferred SSE envelope。内部 model call 会记录原始 OpenAI-compatible Chat Completions SSE cassette。
