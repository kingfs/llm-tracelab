...
开启后，serve 启动会对启用的 YAML upstream target 做一次 probe，并只在内存配置中填补缺失的 `api_type`、`protocol_family` 和未声明的 capability bool；不会写回 YAML。显式配置的 `api_type`、`protocol_family` 或 capability 值不会被覆盖，probe 建议不一致时只记录 warning/log。
