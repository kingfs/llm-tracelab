...
- `audit tool-calls`：durable `tool_call_audits` 查询已支持 status/tool_type/tool_name/executor/call_id selector 和 `--latest-by-call` lifecycle 聚合摘要；CLI 输出限制为 payload summaries、redacted error summary、metadata/count/status，不输出 raw arguments/output/error。
