...
1. Runtime：完整 context optimization/compact v2、复杂 stream tool lifecycle、`file_search`/`code_interpreter`/`computer_use` 执行器安全边界（MCP hosted tool executor 已实现并接线）。
...
- 每个阶段合入后至少跑对应 targeted tests、`git diff --check` 和 `task check:quick`。
...
依赖：现有 `api_type` / capabilities 路由约束（`mode` 仅解析、校验并记录，不参与路由）。
