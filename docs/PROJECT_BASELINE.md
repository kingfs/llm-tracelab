...
- `responses_server.model_profiles` 支持按 `name` 或 `pattern` 匹配 model，声明 `context_window_tokens`、`max_output_tokens`、`compact_history_item_threshold` 和 `upstream_model`。当前 runtime 会使用匹配 profile 的 `compact_history_item_threshold` 覆盖全局 item-count 自动 compact 阈值；配置 `upstream_model` 时，内部 Chat Completions 请求使用该上游模型名，但外部 Responses `model` 仍保留客户端请求 model 或默认 model。尚未实现 token estimator 或完整 context window budgeting。
- 开启 `tools.web_search.enabled=true` 后，非流式 Responses runtime 可执行 hosted `web_search` / `web_search_preview` 首切，provider 支持 `mock` 和 SearXNG；有 ent-backed audit store 时会写 hosted web_search `response.tool_call` started/completed/failed events。普通 `function` tool 默认仍走客户端回路；runtime 现在提供默认空的 server-side function executor registry，调用方显式注册同名 executor 后才会自动执行并写 started/completed/failed events。
...
- function executor 的 YAML/Monitor 配置、streaming tool events 和完整 model profile/context window/token budgeting；当前仅有默认空 executor registry、profile 配置骨架和 item-count compact 阈值覆盖。
