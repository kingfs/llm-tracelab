...
- 多上游和模型/渠道管理。
- 始终可用的本地 Responses runtime（无开关，按模型在 native 与本地 runtime 间二选一）。
- Monitor Web。
- MCP 排障工具。
...
- 本地 Responses runtime 内部调用上游 Chat Completions 时，该上游 HTTP exchange 也写入 `.http` cassette；Responses semantic state 不替代 raw cassette。
...
未来工作：后续接入顺序建议继续补 Responses audit 更完整关联入口，再补 hosted/server-side streaming lifecycle 细化和 compact events。
