...
- Responses audit 语义查询、完整 tool/stream/cancel/compact execution event 写入和完整 Postgres migration 生产化；当前仅覆盖 `request_audits`、内部 `upstream_exchanges` correlation，以及 request/model_call 最小 `execution_events`。
