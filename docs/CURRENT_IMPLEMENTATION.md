...
- 完整 function-tool/stream/cancel/compact execution event 写入和完整 Postgres migration 生产化；当前仅覆盖 `request_audits`、内部 `upstream_exchanges` correlation、request/model_call/hosted web_search 最小 `execution_events`，核心查询服务/Monitor API/MCP/UI 查询，以及 Postgres `db migrate up` 的 versioned SQL 应用路径。
