...
- function executor 的 Monitor 配置、外部 executor 隔离、server-side tool streaming events 和完整 model profile/context optimization；当前已有默认关闭的 YAML `static_response` executor 首切、profile max output/token-budget auto compact 保守估算首切、普通 function call argument streaming 首切，以及已注册 executor 的 stream deferred fallback。
