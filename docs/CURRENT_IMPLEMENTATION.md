...
开启 server-mode 后，当前已支持非流式 Responses 请求经本地 runtime 映射为内部上游 `/v1/chat/completions` 调用；该内部上游 HTTP exchange 会按现有 recorder 写入 `.http` cassette，并在有 ent-backed audit store 时写入一条最小 `upstream_exchanges` correlation。`internal/responses/chatclient` 直接调用 OpenAI-compatible Chat Completions 上游时，已能把 `stream:true` SSE 聚合回内部 `ChatCompletionResponse`，覆盖文本增量、function tool call 参数分片和 usage trailer。server-mode 下游请求 `stream:true` 时，内部 Chat Completions model call 也会请求上游 SSE、把原始 SSE 写入 stream cassette，再聚合为完整 response 供当前 deferred Responses SSE 输出；下游边读边转发和 cancel 传播仍未完成。非 Responses 请求仍走现有代理、路由、录制和解析路径。
...
- `execution_events`：记录 runtime plan、model/tool/compact/stream/error 生命周期事件。当前已写入 request、内部 model_call、hosted `web_search` tool_call、普通 function requested/submitted、deferred stream started/completed、request/model_call cancellation 和 explicit compact 的最小生命周期事件，尚未覆盖下游边转发 streaming 和自动 compact。
...
- 服务端任意 function tool 执行器、streaming tool events 和自动 compact/model profile budgeting。
