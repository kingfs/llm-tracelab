...
- 创建/更新渠道。
- 创建前 provider setup validate 不落库，Save provider 才写入 channel store。
- 探测上游模型。
- 启停渠道。
- 启停模型。
...
- function executor 的 Monitor 写配置、外部 executor root/container 级沙箱、更完整多工具 failed lifecycle events 和完整 model profile/context optimization；当前已有默认关闭的 YAML `static_response` / `external_command` executor 首切、`external_command` opt-in working directory/absolute command 进程隔离首切、只读 Monitor 配置摘要 API 和 Audit 页面状态面板、profile max output/token-budget auto compact estimator adapter 扩展边界和默认保守计数首切、普通 function call argument streaming 首切，以及已注册 executor 的 stream tool loop started/done/failed item 首切。
