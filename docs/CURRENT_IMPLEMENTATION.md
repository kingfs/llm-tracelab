...
- `execution_events`：记录 runtime plan、model/tool/compact/stream/error 生命周期事件。当前已写入 request、内部 model_call、hosted `web_search` tool_call、普通 function requested/submitted、deferred stream started/completed，以及 request/model_call cancellation 的最小生命周期事件，尚未覆盖下游边转发 streaming 和 compact。
