...
- `execution_events`：记录 runtime plan、model/tool/compact/stream/error 生命周期事件。当前已写入 request、内部 model_call、hosted `web_search` tool_call、普通 function requested/submitted，以及 deferred stream started/completed 的最小生命周期事件，尚未覆盖真实上游增量 streaming、cancel、compact。
