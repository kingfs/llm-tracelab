...
- function executor 的 Monitor 配置、外部 executor 隔离、server-side tool streaming events 和完整 model profile/context window/token budgeting；当前已有默认关闭的 YAML `static_response` executor 首切、profile 配置骨架和 item-count compact 阈值覆盖，普通 function call argument streaming 已有首切。
