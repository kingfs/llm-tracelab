...
- Chat Completions SSE cassette 记录与聚合，Responses streaming 覆盖简单文本、auto compact 后简单文本 continuation、auto compact 后未注册普通 `function` tool arguments、auto compact 后同名 registered executor stream tool loop、普通 function arguments、registered executor 和 hosted `web_search` 的首切路径。
- Runtime incremental stream fallback contract 首切：不支持的工具组合，以及 auto compact 后 hosted/未知工具或非平凡 `tool_choice` 的组合，会在写出任何 SSE 或调用上游前返回可识别 fallback error，并带 deferred fallback reason；HTTP handler 可安全转入 deferred envelope 并写 fallback audit event。
...
- Auto compact streaming：简单文本 continuation、未注册普通 `function` tool arguments 和同名 registered executor stream tool loop 已能在自动 compact 后继续真实增量 streaming；hosted/未知工具或非平凡 `tool_choice` 的 auto compact 组合仍显式 fallback 并可审计，后续需继续收敛复杂路径。
...
- `runtime/stream-fallback-contract`：已完成首切，不支持工具组合、以及 auto compact 后 hosted/未知工具或非平凡 `tool_choice` 的 incremental stream fallback 有 runtime contract tests；auto compact 后简单文本 continuation、未注册普通 `function` tool arguments 和同名 registered executor stream tool loop 已不再 fallback。HTTP handler 既有 fallback audit event 继续覆盖 deferred fallback。后续仍需减少复杂路径 fallback，并补跨轮/混合工具生命周期。
