...
- `runtime/stream-fallback-contract`：已完成首切，不支持工具组合、以及 auto compact 后未知/未实现 hosted 工具或非平凡 `tool_choice` 的 incremental stream fallback 有 runtime contract tests；auto compact 后简单文本 continuation、未注册普通 `function` tool arguments、同名 registered executor stream tool loop 和 provider 就绪 hosted `web_search` / `web_search_preview` 平凡 `tool_choice` tool loop 已不再 fallback。HTTP handler 既有 fallback audit event 继续覆盖 deferred fallback。后续仍需减少复杂路径 fallback，并补跨轮/混合工具生命周期。
